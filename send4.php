...
const EXCHANGE_NAME = 'exchange-routing2';
const QUEUE_NAME1 = 'routing3';
const QUEUE_NAME2 = 'routing4';
...
// 5、 绑定交换机和队列，使用的是direct类型的交换机，绑定方式是直接绑定
$channel->queue_bind(QUEUE_NAME1, EXCHANGE_NAME,'ORANGE');
$channel->queue_bind(QUEUE_NAME2, EXCHANGE_NAME,'BLACK');
$channel->queue_bind(QUEUE_NAME2, EXCHANGE_NAME,'GREEN');
// 6、 发消息到交换机
for ($i = 0; $i < 1; $i++) {
    $msg = new AMQPMessage('大橙子!'.$i);
    $channel->basic_publish($msg, EXCHANGE_NAME, 'ORANGE');
    $msg2 = new AMQPMessage('大黑子!'.$i);
    $channel->basic_publish($msg2, EXCHANGE_NAME, 'BLACK');
    $channel->basic_publish($msg, EXCHANGE_NAME, 'BLACK1');
